 ...
 */
export function parseNaturalWhenToDate(
  whenInput: string | undefined,
  baseDate: Date = new Date(),
  tz: string = NY_TZ
): Date {
  if (!whenInput || !whenInput.trim()) return baseDate;

  const normalized = whenInput.trim().toLowerCase()
    .replace(/\s+/g, ' ')
    .replace(/text week/g, 'next week'); // common typo handling

  if (normalized === 'now') return baseDate;
  ...
  const baseInTZ = toZonedTime(baseDate, tz);

  const has = (word: string) => normalized.includes(word);
  const pickHour = (): number => {
    for (const key of Object.keys(PERIODS)) {
      if (has(key)) return PERIODS[key];
    }
    // default time-of-day when unspecified
    return 9; // 09:00 local
  };

  const buildLocal = (y: number, m: number, d: number, h: number) => new Date(y, m, d, h, 0, 0, 0);
  ...
  if (has('today') || has('tonight')) {
    ...
    if (toZonedTime(utc, tz) <= baseInTZ) {
      const bump = new Date(local);
      bump.setDate(bump.getDate() + 1);
      return fromZonedTime(bump, tz);
    }
    return utc;
  }
  ...
  if (has('next week')) {
    ...
    return fromZonedTime(nextMonLocal, tz);
  }
  ...
  for (const key of Object.keys(WEEKDAYS)) {
    if (normalized.startsWith(key)) {
      ...
      let utc = fromZonedTime(tentativeLocal, tz);
      if (toZonedTime(utc, tz) <= baseInTZ) {
        const bump = new Date(tentativeLocal);
        bump.setDate(bump.getDate() + 7);
        utc = fromZonedTime(bump, tz);
      }
      return utc;
    }
  }
...
export function parseNaturalWhenToISO(
  whenInput: string | undefined,
  baseDate: Date = new Date(),
  tz: string = NY_TZ
): string {
